...
class AeScapeNewTab {
  ...
  getSVGIcon(type) {
    const icons = {
      sun: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
      ...
      moon: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path>
      </svg>`,
      cloud: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"></path>
      </svg>`,
      rain: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <line x1="16" y1="13" x2="16" y2="21"></line>
        <line x1="8" y1="13" x2="8" y2="21"></line>
        <line x1="12" y1="15" x2="12" y2="23"></line>
        <path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"></path>
      </svg>`,
      snow: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <line x1="2" y1="17" x2="22" y2="17"></line>
        <line x1="2" y1="12" x2="22" y2="12"></line>
        <line x1="2" y1="7" x2="22" y2="7"></line>
        <path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"></path>
      </svg>`,
      fog: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M3 15h18M3 9h18M3 21h18"></path>
      </svg>`,
      thunderstorm: `<svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
  ...
  async loadWeatherData() {
    try {
      ...
      if (response?.success && response?.data) {
        ...
        this.updateWeatherTheme(response.data);
      }
  ...
  updateWeatherUI(weather) {
    ...
    if (elements.icon) {
      const iconType = weather.env?.isNight ? 'moon' : weather.weather?.code || 'sun';
      elements.icon.innerHTML = this.getSVGIcon(iconType);
    }
  ...
  updateWeatherTheme(weather) {
    ...
    const isNight = weather.env?.isNight;
    const hour = new Date().getHours();

    // 移除所有天气主题类
    body.classList.remove('weather-clear', 'weather-cloudy', 'weather-rain', 
                         'weather-snow', 'weather-night', 'weather-golden');

    // 根据天气和时间应用主题
    if (isNight) {
      body.classList.add('weather-night');
    } else if (hour >= 17 && hour <= 19) {
      body.classList.add('weather-golden');
    } else {
      switch (weatherCode) {
        case 'rain':
        case 'thunderstorm':
  ...
  showErrorState() {
    ...
    if (elements.icon) elements.icon.innerHTML = this.getSVGIcon('cloud');
  }
