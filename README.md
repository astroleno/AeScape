...
3. 开启"开发者模式"
4. 点击"加载已解压的扩展程序"
5. 选择 `video_intergration` 文件夹
6. 详细安装指南请参考：[video_intergration/INSTALL.md](video_intergration/INSTALL.md)
...
```
AeScape/
├── chrome_store/          # 主插件（基础版）
├── video_intergration/    # 整合版插件（推荐）
├── video_module/          # 视频模块（独立开发）
...
- **chrome_store/** - 基础版插件，包含天气显示和悬浮球功能
- **video_intergration/** - 整合版插件，融合了视频动画系统（推荐使用）
- **video_module/** - 独立的视频模块，可单独使用或集成到其他项目
...
### v1.2.2 (2024-12-19)
- 🚀 **新增优化模块系统**：
  - **错误处理器** (`ErrorHandler.js`)：统一错误处理、日志记录和用户通知
  - **性能监控器** (`PerformanceMonitor.js`)：监控扩展性能指标，包括内存使用、API响应时间等
  - **配置管理器** (`ConfigManager.js`)：统一管理所有配置项，提供验证和持久化
  - **视频资源管理器** (`VideoResourceManager.js`)：智能管理视频预加载、缓存和资源释放
  - **智能主题适配器** (`SmartThemeAdapter.js`)：优化主题切换性能，减少重复计算和DOM操作

- 🔇 **用户体验优化**：
  - 将"扩展上下文不可用"等技术警告改为友好的调试信息
  - 启用静默模式，减少不必要的控制台输出
  - 优化错误信息显示，使用更用户友好的语言
...
- 🛠️ **开发者工具**：
  - 新增 `test-optimization.html` 测试页面，可验证所有优化模块功能
  - 完善的性能统计和错误跟踪
  - 模块化设计，便于维护和扩展

### v1.2.1 (2024-12-19)
- 🐛 **修复悬浮球问题**：
  - 解决悬浮球疯狂连续请求主题色的问题（优化主题监听器，避免重复注册）
  - 修复关闭打开页面时出现多个悬浮球实例的问题（改进实例清理机制）
  - 统一主题监听器管理，提升性能和稳定性

- 🎨 **优化卡片显示**：
  - 调整天气详情卡片字号，使其更加适中易读
  - 优化卡片排版布局，保持与基础版本一致的视觉风格
  - 改进数据项的视觉层次和间距
