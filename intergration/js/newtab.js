...
class AeScapeNewTab {
  ...
  initVideoModules() {
    try {
      ...
      // 检查必要的类是否可用
      if (typeof UnifiedThemeSystem === 'undefined') {
        console.warn('UnifiedThemeSystem not available');
        return;
      }
      
      if (typeof VideoWeatherManager === 'undefined') {
        console.warn('VideoWeatherManager not available');
        return;
      }
      
      // 初始化主题系统
      this.themeSystem = new UnifiedThemeSystem();
      
      // 初始化天气视频管理器
      this.weatherManager = new VideoWeatherManager({
        duration: 1500,
        enhanceEffects: true,
        autoPlay: true,
        onStart: () => {
          console.log('Weather video started');
        },
        onEnd: () => {
          console.log('Weather video ended');
        },
        onError: (error) => {
          console.error('Weather video error:', error);
        },
        onWeatherChange: (weatherType, videoInfo) => {
          console.log('Weather video changed:', weatherType);
        }
      });
      
      this.weatherManager.init('intro-video-container');
      
      // 初始化沉浸式增强管理器
      if (typeof ImmersiveEnhancementManager !== 'undefined' && this.themeSystem) {
        this.immersiveEnhancer = new ImmersiveEnhancementManager(this.weatherManager, this.themeSystem);
        console.log('Immersive enhancement manager initialized');
      }
      
      console.log('Video modules initialized successfully');
    } catch (error) {
      console.error('Failed to initialize video modules:', error);
    }
  }
  ...
  updateWeatherUI(weather) {
    ...
    // 触发视频播放
    this.triggerWeatherVideo(weather);
  }
  ...
  // 触发天气视频播放
  async triggerWeatherVideo(weather) {
    try {
      // 检查视频管理器是否初始化
      if (!this.weatherManager || !this.themeSystem) {
        console.log('Video modules not initialized, skipping video');
        return;
      }
      ...
      // 检查是否应该播放视频（避免重复播放）
      if (this.hasVideoTriggered && this.lastWeatherType === weatherCode) {
        console.log('Video already triggered for this weather type');
        return;
      }

      console.log('Triggering weather video:', weatherCode, 'isNight:', isNight);

      // 获取时间段
      const timeSlot = this.getTimeSlot(currentHour, isNight);
      
      // 使用沉浸式增强播放（如果可用）
      if (this.immersiveEnhancer) {
        await this.immersiveEnhancer.enhanceWeatherExperience(
          weatherCode,
          timeSlot,
          weather?.env?.temperature || 20,
          {
            intensity: 'medium',
            enableRitual: true,
        ...
        // 回退到基础视频播放
        await this.weatherManager.playWeatherVideo(weatherCode, {
          intensity: 'medium'
        });
      }

      // 更新状态
      this.hasVideoTriggered = true;
      this.lastWeatherType = weatherCode;
      ...
      console.error('Failed to trigger weather video:', error);
    }
  }

  // 获取时间段
  getTimeSlot(hour, isNight) {
